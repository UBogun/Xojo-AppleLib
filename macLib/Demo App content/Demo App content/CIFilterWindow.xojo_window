...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  dim grad as new AppleCIGaussianGradient(150, 400, &cD9FFF400, &c80092800, 100)
		  
		  dim histo as new AppleCIAreaHistogram(nil,0,0,0,0,256, 10)
		  ChainFilters(grad, histo)
		  histo.InputExtent = grad.OutputCIImage.Extent
		  filter = new AppleCIHistogramDisplayFilter (histo, 200)
		  dim ai as AppleImage = filter.OutputciImage.toappleimage(OSXLibView1.width, osxlibview2.width, true)
		  // // filter  = myfilter
		  
		  OSXLibView1.AppleObject.Image = ai
		  ShowAttribues
		...
		Sub Change()
		  try
		    ...
		    case "HistogramDisplayFilter"
		      dim prefilter as new AppleCIAreaHistogram(pic,0,0,0,0,256)
		      filter = new AppleCIHistogramDisplayFilter (prefilter, 200)
